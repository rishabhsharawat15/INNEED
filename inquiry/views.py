...
def inquirys(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            inquirys1 = inquiry(listing=listing, listing_id=listing_id, name=name, email=email,
                                phone=phone, message=message, user_id=user_id, owner_id=owner_id)
            inquirys1.save()
            send_mail(
                'Inquiry for ' + listing,
                'There has been an inquiry for ' + listing +
                '.Sign in to your dashboard for further info',
                'kartik200212@gmail.com',
                [owner_mail],
                fail_silently=False
            )
            messages.success(
